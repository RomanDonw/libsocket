...
extern const char *LIBSOCKET_MODULENAME;

LIBSOCKET_API bool LIBSOCKET_ABI libsocket_initialized(void); // can be accessed without library initialization.
LIBSOCKET_API NError LIBSOCKET_ABI libsocket_startup(const LibSocketStartupOptions *options, LibSocketStartupResults *results); // options & results can be NULL.
LIBSOCKET_API NError LIBSOCKET_ABI libsocket_cleanup(void);

LIBSOCKET_API NError LIBSOCKET_ABI socket_parseipaddr(IPAddressInterface *addr, SocketAddressFamily af, const char *straddr);
LIBSOCKET_API NError LIBSOCKET_ABI socket_ipaddrtostr(const IPAddressInterface *addr, SocketAddressFamily af, char *straddr, socklen_t size);

// [socket_packsockipaddr]: can be accessed without library initialization.
LIBSOCKET_API NError LIBSOCKET_ABI socket_packsockipaddr(SocketIPAddressInterface *sockaddr, SocketAddressFamily af, const IPAddressInterface *addr, unsigned short port);
// [socket_unpacksockipaddr]: can be accessed without library initialization.
LIBSOCKET_API NError LIBSOCKET_ABI socket_unpacksockipaddr(const SocketIPAddressInterface *sockaddr, SocketAddressFamily af, IPAddressInterface *addr, unsigned short *port);
...
LIBSOCKET_API NError LIBSOCKET_ABI socket_getaddrinfo(const char *hostname, const char *servicename, const SocketDNSRequest *request, SocketDNSResponse **response);
...
LIBSOCKET_API NError LIBSOCKET_ABI socket_getnameinfo(const SocketIPAddressInterface *sockaddr, socklen_t sockaddrlen, char *hostname, size_t *hostnamesize, char *servicename, size_t *servicenamesize, SocketGetNameInfoFlags flags);

LIBSOCKET_API NError LIBSOCKET_ABI socket_open(Socket **socket, SocketAddressFamily af, SocketType type, SocketProtocol protocol);
LIBSOCKET_API NError LIBSOCKET_ABI socket_close(Socket *socket);

LIBSOCKET_API NError LIBSOCKET_ABI socket_connect(const Socket *socket, const SocketAddressInterface *sockaddr, socklen_t sockaddrlen);
LIBSOCKET_API NError LIBSOCKET_ABI socket_bind(const Socket *socket, const SocketAddressInterface *sockaddr, socklen_t sockaddrlen);

LIBSOCKET_API NError LIBSOCKET_ABI socket_listen(const Socket *socket, int backlog);
// [socket_accept]: sockaddr & sockaddrlen can be NULL. see <sys/socket.h> accept function documentation for more info.
LIBSOCKET_API NError LIBSOCKET_ABI socket_accept(Socket **acceptedsocket, const Socket *socket, SocketAddressInterface *sockaddr, socklen_t *sockaddrlen);

// [socket_recv - ... - socket_sendto]: processedbytes can be NULL.
LIBSOCKET_API NError LIBSOCKET_ABI socket_recv(const Socket *socket, void *buffer, size_t size, size_t *processedbytes, int flags);
// [socket_recvfrom]: sockaddr & sockaddrlen can be NULL. see <sys/socket.h> recvfrom function documentation for more info.
LIBSOCKET_API NError LIBSOCKET_ABI socket_recvfrom(const Socket *socket, void *buffer, size_t size, size_t *processedbytes, int flags, SocketAddressInterface *sockaddr, socklen_t *sockaddrlen);
LIBSOCKET_API NError LIBSOCKET_ABI socket_send(const Socket *socket, const void *data, size_t size, size_t *processedbytes, int flags);
LIBSOCKET_API NError LIBSOCKET_ABI socket_sendto(const Socket *socket, const void *buffer, size_t size, size_t *processedbytes, int flags, const SocketAddressInterface *sockaddr, socklen_t sockaddrlen);

LIBSOCKET_API bool LIBSOCKET_ABI socket_isnonblocking(const Socket *socket); // can be accessed without library initialization.
LIBSOCKET_API NError LIBSOCKET_ABI socket_setnonblocking(Socket *socket, bool enable);
LIBSOCKET_API NError LIBSOCKET_ABI socket_getreadablebytes(const Socket *socket, size_t *availbytes);
LIBSOCKET_API NError LIBSOCKET_ABI socket_shutdown(const Socket *socket, SocketShutdownFlags flags);

LIBSOCKET_API NError LIBSOCKET_ABI socket_getopt(const Socket *socket, SocketOptionLevel level, SocketOptionName optname, void *optval, size_t *optsize);
LIBSOCKET_API NError LIBSOCKET_ABI socket_setopt(const Socket *socket, SocketOptionLevel level, SocketOptionName optname, const void *optval, size_t optsize);
...
LIBSOCKET_API NError LIBSOCKET_ABI socket_getpeername(const Socket *socket, SocketAddressInterface *sockaddr, socklen_t *size);
LIBSOCKET_API NError LIBSOCKET_ABI socket_getsockname(const Socket *socket, SocketAddressInterface *sockaddr, socklen_t *size);
