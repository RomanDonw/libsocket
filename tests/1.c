...
void printipv4(IPv4Address addr, const char *addrname)
{
    ...
    if ((err = socket_ipaddrtostr(&addr, SocketAddressFamily_IPv4, addrstr, IPV4ADDRSTRSIZE)) != NError_Success) handlesockerror(err, "socket_addrtostr");
    printf("%s: %s\n", addrname, addrstr);
}
...
void test(void)
{
...

    {
        ...
        if ((err = socket_packsockipaddr(&saddr, SocketAddressFamily_IPv4, &addr, port)) != NError_Success) handlesockerror(err, "socket_packsockaddr");
        if ((err = socket_bind(s, &saddr, sizeof(saddr))) != NError_Success) handlesockerror(err, "socket_bind");
        ...
        if ((err = socket_unpacksockipaddr(&saddr, SocketAddressFamily_IPv4, &addr, &port)) != NError_Success) handlesockerror(err, "socket_unpacksockaddr");
        if ((err = socket_ipaddrtostr(&addr, SocketAddressFamily_IPv4, addrstr, IPV4ADDRSTRSIZE)) != NError_Success) handlesockerror(err, "socket_addrtostr");
    ...
    if ((err = socket_setopt(s, SocketOptionLevel_Socket, SocketOptionName_Socket_Linger, &ling, sizeof(ling))) != NError_Success) handlesockerror(err, "socket_setopt");

    ling.enable = false;
    ling.linger = 666;

    size_t lingsz = sizeof(SocketLingerOptions);
    if ((err = socket_getopt(s, SocketOptionLevel_Socket, SocketOptionName_Socket_Linger, &ling, &lingsz)) != NError_Success) handlesockerror(err, "socket_getopt");
